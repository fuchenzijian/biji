...
function Person(){ }
var p = new Person();
p.toString(); // 实际上调用的是Object.prototype.toStr
ing( )
```
...
## //访问一个变量的值,js优先会从他的作用域(方法体)中找,如果当前作用域没有

## //他会向他的父级作用域去找,一直找到window;
...
消耗内存：通常来说，函数的活动对象会随着执行上下文环境一起被销毁，但是由于闭包引用的是外部函数的活动对象，因此这个活动对象无法被销毁，所以说，闭包比一般的函数需要消耗更多的内存。  内存泄漏(现在没有)  不会被垃圾回收机制回收;
